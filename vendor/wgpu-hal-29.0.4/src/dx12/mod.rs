...
impl crate::Surface for Surface {
    ...
    unsafe fn configure(
        ...
        // Upstream sets this unconditionally. `DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING`
        // is documented by Microsoft as valid only for swapchains created via
        // `CreateSwapChainForHwnd` — every other target below (`Visual`,
        // `VisualFromWndHandle`, `SurfaceHandle`, `SwapChainPanel`) creates its
        // swapchain via `CreateSwapChainForComposition`/
        // `CreateSwapChainForCompositionSurfaceHandle` instead, and passing this
        // flag there makes swapchain creation fail outright with
        // `DXGI_ERROR_INVALID_CALL` (0x887A0001) on any adapter that reports
        // tearing support — which is effectively every modern GPU. Confirmed by
        // reproducing the exact failure with `Dx12SwapchainKind::DxgiFromVisual`.
        let target_supports_tearing = matches!(self.target, SurfaceTarget::WndHandle(_));
        if self.supports_allow_tearing && target_supports_tearing {
        ...
        let swap_chain = match self.swap_chain.write().take() {
            ...
            Some(sc) => {
                ...
                // --- local patch (see vendor/README.md) ---
                // The comment right above this match arm already flags it as
                // incomplete. Concretely, for a DirectComposition-backed
                // swapchain (`VisualFromWndHandle`): `ResizeBuffers` resizes
                // the DXGI buffers, but the compositor is never told about
                // it — nothing here calls `IDCompositionDevice::Commit`
                // again, and `Commit` is what actually pushes a pending
                // change (including "the content this visual displays is
                // now a different size") out to the desktop compositor.
                // Reproduced exactly the visual symptom this produces:
                // after resizing the window larger, the original rect kept
                // showing whatever was last committed (frozen), while the
                // newly exposed area — never covered by any committed
                // visual content — showed raw desktop passthrough instead
                // of this app's own background.
                if let SurfaceTarget::VisualFromWndHandle { handle, dcomp_state } = &self.target {
                    // Confirmed via temporary `warn`-level diagnostics
                    // (see vendor/README.md) that this branch runs and both
                    // calls below succeed on every resize — downgraded to
                    // `debug` now that's established; re-promote to `warn`
                    // if this ever needs re-diagnosing.
                    log::debug!(
                        "[local patch] resize configure: re-committing DirectComposition visual to {}x{}",
                        config.extent.width,
                        config.extent.height
                    );
                    let mut dcomp_state_guard = dcomp_state.lock();
                    match unsafe { dcomp_state_guard.get_or_init(&device.dcomp_lib, handle) } {
                        Ok(inner) => {
                            if let Err(err) = unsafe { inner.visual.SetContent(&raw) } {
                                log::error!(
                                    "IDCompositionVisual::SetContent (resize) failed: {err}"
                                );
                            }
                            if let Err(err) = unsafe { inner.device.Commit() } {
                                log::error!("IDCompositionDevice::Commit (resize) failed: {err}");
                            }
                        }
                        Err(err) => {
                            log::error!(
                                "Failed to re-acquire DirectComposition state on resize: {err}"
                            );
                        }
                    }
                }
